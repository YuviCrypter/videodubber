...
export class Store {
  ...
  constructor() {
    ...
    this.fps = 60;
    this.animations = [];
    this.animationTimeLine = typeof window != "undefined" && anime.timeline();
    this.selectedMenuOption = "Media";
  ...
  addVideo(index) {
    ...
    if (!isHtmlVideoElement(videoElement)) {
      console.log("Failed to get html video element");
      return;
    }
    ...
    for (let index = 0; index < store.editorElements.length; index++) {
      ...
      switch (element.type) {
        case "video": {
          ...
          // console.log("video obj", videoObject);
